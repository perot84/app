        ...
        fig.add_trace(go.Bar(
            ...
            marker=dict(
                ...
                pattern=dict(
                    ...
                    size=16,  # Ajusta este valor para cambiar la distancia entre líneas
                    solidity=0.3  # Ajusta este valor para cambiar la opacidad del patrón
                )
            ),
            width=0.5,
            showlegend=False,
...
def create_table(df_subset, project_id):
    ...
    # Devolver el título del proyecto, las fechas, la tabla y el botón
...
for project_id in df['ID_Projecte'].unique():
...
project_ids = df['ID_Projecte'].unique()
for project_id in project_ids:
    df_filtered = df[df['ID_Projecte'] == project_id].drop(columns=['ID_Projecte'])
